...
trait ProvideContextOps extends ProvideCellId with ElaboraterBase {
  ...
  implicit class TyAndValnuoOpsss(ignored: TyAndVal.type) {
    ...
    def create()(using state: StateOps[TyckSession]): TyAndVal =
      new TyAndVal(toTerm(state.addCell(OnceCell[Term]())), toTerm(state.addCell(OnceCell[Term]())))
  }

  extension (context: ContextItem) {
    def tyId(using StateOps[TyckSession]): CellId[Term] = toId(context.ty)

    def tyTerm(using StateOps[TyckSession]): Term = toTerm(context.ty)
  }

  implicit class ContextItemObject(ignored: ContextItem.type) {
    def builtin(
        item: BuiltinItem
    )(using state: StateOps[TyckSession]): (TyAndVal, ContextItem) = {
  ...
  implicit class TyAndValOps(tyandval: TyAndVal) {
    def tyId(using StateOps[TyckSession]): CellId[Term] = toId(tyandval.ty)

    def valueId(using StateOps[TyckSession]): CellId[Term] = toId(tyandval.value)

    def tyTerm(using StateOps[TyckSession]): Term = toTerm(tyandval.ty)

    def valueTerm(using StateOps[TyckSession]): Term = toTerm(tyandval.value)

  }

  implicit class LocalCtxOps(ignored: Context.type) {
    def default(using StateOps[TyckSession]): Context = {
