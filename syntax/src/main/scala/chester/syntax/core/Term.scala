...
) extends WHNFTerm derives ReadWriter {
...
sealed trait MetaTermA extends MoreTermA {
  override type ThisTree <: MetaTermA
  def impl: MetaTermHold[?]
}

case class MetaTerm(impl: MetaTermHold[?], meta: OptionTermMeta = None) extends MoreTerm with MetaTermA {
...
sealed trait ListTermA extends WHNFTermA {
  override type ThisTree <: ListTermA
  def terms: Vector[TermA]
}

case class ListTerm(terms: Vector[Term], meta: OptionTermMeta = None) extends WHNFTerm with ListTermA derives ReadWriter {
...
sealed trait TypeTermA extends WHNFTermA { 
  override type ThisTree <: TypeTermA
}

sealed trait TypeTerm extends WHNFTerm with TypeTermA derives ReadWriter {
...
sealed trait Level extends WHNFTerm derives ReadWriter {
  type ThisTree <: Level
}
...
sealed trait LiteralTerm extends WHNFTerm derives ReadWriter
...
case class Function(ty: FunctionType, body: Term, meta: OptionTermMeta = None) extends WHNFTerm {
...
sealed trait Builtin extends WHNFTerm derives ReadWriter {
...
sealed trait Constructed extends WHNFTerm derives ReadWriter {
...
/** Effect needs to have reasonable equals and hashcode for simple comparison, whereas they are not requirements for other Terms
  */
sealed trait Effect extends WHNFTerm derives ReadWriter {
...
case class Effects(effects: Map[LocalV, Term], meta: OptionTermMeta = None) extends WHNFTerm derives ReadWriter {
...
sealed trait ReferenceCall extends UnevaluatedTerm with TermWithUniqId derives ReadWriter {
...
case class ErrorTerm(problem: Problem, meta: OptionTermMeta = None) extends MoreTerm {
...
sealed trait StmtTerm extends WHNFTerm derives ReadWriter {
...
case class TupleTerm(values: Vector[Term], meta: OptionTermMeta = None) extends WHNFTerm {
...
case class BlockTerm(
    ...
    value: Term,
    meta: OptionTermMeta = None
) extends UnevaluatedTerm derives ReadWriter {
...
case class TodoTerm(meta: OptionTermMeta = None) extends MoreTerm {
