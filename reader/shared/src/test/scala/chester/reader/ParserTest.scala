...
class ParserTest extends FunSuite {
  test("parse valid identifier") {
    val result =
      reporterToEither(ChesterReaderV1.parseExpr(FileNameAndContent("testFile", "validIdentifier123")))
    result match {
  ...
  test("parse identifier with symbols") {
    val result =
      reporterToEither(ChesterReaderV1.parseExpr(FileNameAndContent("testFile", "valid-Identifier_123")))
    result match {
      case Right(Identifier(name, Some(meta))) =>
  ...
  test("parse empty input") {
    val result = reporterToEither(ChesterReaderV1.parseExpr(FileNameAndContent("testFile", "")))
  ...
  test("parse single-line string literal") {
    ...
    val result = reporterToEither(ChesterReaderV1.parseExpr(FileNameAndContent("testFile", input)))
    result match {
