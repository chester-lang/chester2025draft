...
class Tokenizer(src: SourceOffset) {
  ...
  private def mkPos(start: Int, end: Int = pos) = SourcePos(src, RangeInFile(
    Pos(WithUTF16(start.refineUnsafe[Positive0], text.substring(0, start).length.refineUnsafe[Positive0]), 
        line.refineUnsafe[Positive0], WithUTF16(0.refineUnsafe, col.refineUnsafe[Positive0])),
    Pos(WithUTF16(end.refineUnsafe[Positive0], text.substring(0, end).length.refineUnsafe[Positive0]), 
        line.refineUnsafe[Positive0], WithUTF16((end - start).refineUnsafe, 
        (text.substring(0, end).length - text.substring(0, start).length).refineUnsafe))
  ))
    ...
    if (start >= text.length) err("Unexpected end of input in escape sequence", start - 1)
      ...
      case 'u' if start + 4 < text.length =>
        ...
        Try(Integer.parseInt(hex, 16))
          ...
          .toEither.left.map(_ => ParseError(s"Invalid Unicode escape \\u$hex", mkPos(start - 1, start + 5).range.start))
      case 'x' if start + 2 < text.length =>
        val hex = text.substring(start + 1, start + 3)
        Try(Integer.parseInt(hex, 16))
          .map(v => (v.toChar.toString, start + 3))
          .toEither.left.map(_ => ParseError(s"Invalid hex escape \\x$hex", mkPos(start - 1, start + 3).range.start))
      case c if c >= '0' && c <= '7' =>
        ...
        Try(Integer.parseInt(text.substring(start, end), 8))
          ...
          .toEither.left.map(_ => ParseError(s"Invalid octal escape \\${text.substring(start, end)}", mkPos(start - 1, end).range.start))
  ...
  private def parseIdent(initial: String, start: Int): Either[ParseError, Token] = {
    ...
    while (pos < text.length && (text.charAt(pos).isLetterOrDigit || text.charAt(pos) == '_' || 
