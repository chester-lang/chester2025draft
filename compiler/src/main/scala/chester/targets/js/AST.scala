...
case class Identifier(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text(name)
}
...
case class TypedIdentifier(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text(name) <> Doc.text(":") <+> typeAnnotation.toDoc
}
...
case class ConditionalExpression(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.group(test.toDoc <+> Doc.text("?") <+> consequent.toDoc <+> Doc.text(":") <+> alternate.toDoc)
}
...
case class NumericLiteral(value: Double) extends Literal {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text(value.toString)
}

case class StringLiteral(value: String) extends Literal {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("\"" + value + "\"")
}

case class BooleanLiteral(value: Boolean) extends Literal {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text(if (value) "true" else "false")
}

case object NullLiteral extends Literal {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("null")
}

case class BigIntLiteral(value: String) extends Literal {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text(value + "n")
}

case class RegExpLiteral(pattern: String, flags: String) extends Literal {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text(s"/$pattern/$flags")
}
...
case class OptionalMemberExpression(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = MemberExpression(obj, property, computed, optional = true).toDoc
}
...
case class AwaitExpression(argument: Expression) extends Expression {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("await") <+> argument.toDoc
}
...
case class TaggedTemplateExpression(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = tag.toDoc <> quasi.toDoc
}

case class SequenceExpression(expressions: List[Expression]) extends Expression {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.group(Doc.sep(Doc.text(","), expressions.map(_.toDoc)))
}

case class SpreadElement(argument: Expression) extends Expression {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("...") <> argument.toDoc
}
...
case class AssignmentPattern(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.group(left.toDoc <+> Doc.text("=") <+> right.toDoc)
}
...
case class VariableDeclarator(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = init match {
...
case class Parameter(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = id.toDoc
...
case class ExpressionStatement(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = expression.toDoc <> Doc.text(";")
}
...
case class WhileStatement(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("while (") <> test.toDoc <> Doc.text(")") <+> body.toDoc
}
...
case class DoWhileStatement(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("do") <+> body.toDoc <+> Doc.text("while (") <> test.toDoc <> Doc.text(");")
}
...
case class ForInStatement(
  ...
  def leftDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = left match {
    case Left(varDecl) => varDecl.toDoc
    case Right(expr)   => expr.toDoc
  }
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("for (") <> leftDoc <> Doc.text(" in ") <> right.toDoc <> Doc.text(")") <+> body.toDoc
}
...
case class ForOfStatement(
  ...
  def awaitDoc(implicit options: PrettierOptions = PrettierOptions.Default) = if (await) Doc.text("await") <+> Doc.empty else Doc.empty
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("for") <+> awaitDoc <> Doc.text("(") <> leftDoc <> Doc.text(" of ") <> right.toDoc <> Doc.text(")") <+> body.toDoc
}
...
case class BreakStatement(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = label match {
...
case class ReturnStatement(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = argument match {
...
case class ThrowStatement(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("throw") <+> argument.toDoc <> Doc.text(";")
}
...
case class ClassDeclaration(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = ClassExpression(Some(id), typeParameters, superClass, implementsInterfaces, body, decorators).toDoc
}
...
case class ImportDefaultSpecifier(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = local.toDoc
}

case class ImportNamespaceSpecifier(
  local: Identifier
) extends ImportSpecifier {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("* as ") <> local.toDoc
}
...
case class ExportDefaultDeclaration(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("export default ") <> declaration.toDoc
}
...
case class UnionType(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.sep(Doc.text(" | "), types.map(_.toDoc))
}
...
case class GenericTypeAnnotation(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = TypeReference(id.name, typeParameters).toDoc
}

// Primitive Type Annotations
case object AnyTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("any")
}

case object UnknownTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("unknown")
}

case object NumberTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("number")
}

case object StringTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("string")
}

case object BooleanTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("boolean")
}
...
case object UndefinedTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("undefined")
}

case object NeverTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("never")
}

case object ObjectTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("object")
}

case object SymbolTypeAnnotation extends TypeAnnotation {
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("symbol")
}
...
case class EnumMember(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = initializer match {
...
case class TypeAssertion(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = Doc.text("<") <> typeAnnotation.toDoc <> Doc.text(">") <> expression.toDoc
}
...
case class AsExpression(
  ...
  def toDoc(implicit options: PrettierOptions = PrettierOptions.Default): Doc = expression.toDoc <+> Doc.text("as ") <> typeAnnotation.toDoc
}
